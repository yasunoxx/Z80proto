...
#else   // Z80proto
#ifdef NOWDEBUG
extern int16_t get_SIO0_polling( void );
#else
uint8_t get_SIO0_polling( void )
{
    register int16_t tbuf;
    while( 1 )
    {
        tbuf = ( int16_t )getchar_SIO0();
        if( tbuf == 0 ) // NUL
        {
            tbuf = BUF_GETCHAR_SIO0;
            if( tbuf == 0 ) // real NUL
            {
                return ( uint8_t )NUL;
            }
...
#else   // Z80proto
        getchar_SIO0();
        register uint16_t tbuf = BUF_GETCHAR_SIO0;
        if( ( tbuf & 0x00FF00 ) == 0 )
        {
            buf = ( uint8_t )tbuf;
            break;
